package com.nam_dev.driving_license_review.utils;
...
public class TextUtils {
    ...
    public static final String USERNAME_REGEX = "^(?=.*[a-z])(?=.*\\d)[a-zA-Z0-9_]{6,16}$";
    public static final String PASSWORD_REGEX = "^(?=.*[a-z])(?=.*\\d)[a-zA-Z0-9]{8,16}$";
    ...
    public static String randomCaptcha(int length) {
        String str = "abcde2345678ghpqzcbsrtukypwx";
    ...
    public static String convertPhoneVN(String phone) {
        if (TextUtils.isEmpty(phone)) return null;
        if (phone.length() < 7) return null;
        if (phone.startsWith("+84") && isSubPhone(phone.substring(3))) {
            return "(+84)" + phone.substring(3);
        }
        if (phone.startsWith("(+84)") && isSubPhone(phone.substring(5))) {
            return "(+84)" + phone.substring(5);
        }
        if (phone.startsWith("0") && isSubPhone(phone.substring(1))) {
            return "(+84)" + phone.substring(1);
        }
        if (phone.startsWith("84") && isSubPhone(phone.substring(2))) {
            return "(+84)" + phone.substring(2);
        }
        if (!phone.startsWith("0") && isSubPhone(phone)) {
            return "(+84)" + phone;
        }
        return null;
    }

    private static boolean isSubPhone(String subPhone) {
        if (9 != subPhone.length()) {
            return false;
        }
        return isIntegerNumber(subPhone);
    }
    ...
    public static Boolean checkUsername(String value) {
        return Pattern.compile(USERNAME_REGEX).matcher(value).matches();
    }
    ...
    public static String generateRandomString(Long userId) {
        SecureRandom random = new SecureRandom();
        int length = 0;
        if (userId.toString().length() >= 8) {
            return userId.toString();
        }
        length = 8 - userId.toString().length();
        ...
        return sb.toString() + userId.toString();
    }
}
