...
import static com.nam_dev.driving_license_review.constants.ImageConstant.*;
...
public class VideoService {
     ...
     */
    public ResponseEntity<byte[]> prepareContent(String path, String range) {
        long rangeStart = 0;
        long rangeEnd;
        byte[] data;
        Long fileSize;
        List<String> split = Arrays.asList(path.split("\\.", 2));
        String fileType = split.get(1);
        try {
            ...
            if (range == null) {
                return ResponseEntity.status(HttpStatus.OK)
                        ...
                        .body(
                                readByteRange(
                                        path, rangeStart, fileSize - 1)); // Read the object and convert it as bytes
            }
